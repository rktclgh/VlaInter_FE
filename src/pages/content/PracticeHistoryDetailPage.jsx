...
import {
  bookmarkInterviewTurn,
  getInterviewSessionResults,
  getTechInterviewHistorySummary,
...
const buildSummaryPills = (summary) => {
  ...
  const items = [];
  const categoryName = String(summary.categoryName || "").trim();
  const jobName = String(summary.jobName || "").trim();
  const languageLabel = summary.language ? getInterviewLanguageLabel(summary.language) : "";

  if (categoryName) items.push({ label: categoryName, accent: true });
  if (jobName) items.push({ label: jobName, accent: false });
  if (languageLabel) items.push({ label: languageLabel, accent: false });
  items.push({ label: `문항 ${summary.questionCount}개`, accent: false });
...
export const PracticeHistoryDetailPage = () => {
  ...
  useEffect(() => {
    ...
    const loadPage = async () => {
      ...
      try {
        ...
        if (!active) return;
        setPageErrorMessage(error?.message || "기술질문 연습 결과를 불러오지 못했습니다.");
      } finally {
    ...
    <HistoryShell activeKey="practice_history">
      <div className="space-y-6">
        <header className="history-fade-up pt-2">
          <button
            type="button"
            onClick={() => navigate("/content/practice-history")}
          ...
          {summaryPills.length ? (
            <div className="mt-4 flex flex-wrap gap-2">
              {summaryPills.map((pill) => (
                <HistoryChip key={pill.label} accent={pill.accent}>
                  {pill.label}
                </HistoryChip>
              ))}
        ...
        {loadingPage ? (
          <div className="rounded-[22px] border border-[#ececec] bg-white px-5 py-10 text-center">
            <InlineSpinner label="기술질문 연습 결과 불러오는 중..." />
          </div>
        ) : (
          <>
